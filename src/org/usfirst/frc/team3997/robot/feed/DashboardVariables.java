package org.usfirst.frc.team3997.robot.feed;

import jaci.pathfinder.Pathfinder;
import jaci.pathfinder.Waypoint;

public class DashboardVariables {

	public static double firstAutoTime = 0;
	
	public static double DRIVE_P = 0.0;
	public static double DRIVE_I = 0.0;
	public static double DRIVE_D = 0.0;
	
	public static double ARM_P = 0.0;
	public static double ARM_I = 0.0;
	public static double ARM_D = 0.0;
	public static double ARM_F = 0.0;

	
	public static double CLIMB_ARM_ANGLE = 0.0;
	public static double SWITCH_ARM_ANGLE = 0.0;
	public static double SCALE_ARM_ANGLE = 0.0;
	public static double FEED_ARM_ANGLE = 0.0;

	
	
	
	public static double max_speed = 1;
	
	//Paths
	
	

}
