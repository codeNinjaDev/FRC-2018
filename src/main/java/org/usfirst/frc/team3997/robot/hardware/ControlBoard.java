...
/** Driver Triggers **/
	...
	public ControlBoard() {
		...
		if (Ports.USING_WIN_DRIVER_STATION) {
			...
			intakeDriverButton = new ButtonReader(driverJoy, XBoxMappings.LEFT_BUMPER, "INTAKE_DRIVER");
			outtakeDriverButton = new ButtonReader(driverJoy, XBoxMappings.RIGHT_BUMPER, "OUTTAKE_DRIVER");
			
			slowDriveTier1Button = new TriggerReader(driverJoy, XBoxMappings.RIGHT_TRIGGER_AXIS, "BRAKE_1");
			slowDriveTier2Button = new TriggerReader(driverJoy, XBoxMappings.LEFT_TRIGGER_AXIS, "BRAKE_2");
			
			//Operator Controls
			armScaleButton = new ButtonReader(operatorJoy, XBoxMappings.YELLOW_BUTTON, "SCALE");
			armSwitchButton = new ButtonReader(operatorJoy, XBoxMappings.GREEN_BUTTON, "SWITCH");
			armFeedButton = new ButtonReader(operatorJoy, XBoxMappings.BLUE_BUTTON, "FEED");
			armManualButton = new ToggleButtonReader(operatorJoy, XBoxMappings.BACK_BUTTON, "ARM_MANUAL");
			
			intakeWheelsButton = new TriggerReader(operatorJoy, XBoxMappings.LEFT_TRIGGER_AXIS, "IN_WHEELS");
			outtakeWheelsButton = new TriggerReader(operatorJoy, XBoxMappings.RIGHT_TRIGGER_AXIS, "OUT_WHEELS");
			intakeOperatorButton = new ButtonReader(operatorJoy, XBoxMappings.RIGHT_BUMPER, "INTAKE_OP");
			outtakeOperatorButton = new ButtonReader(operatorJoy, XBoxMappings.LEFT_BUMPER, "OUTTAKE_OP");
	...
	@Override
	public boolean outtakeWheelsDesired() {
		return outtakeWheelsDesired;
	}
	@Override
	public boolean intakeWheelsDesired() {
